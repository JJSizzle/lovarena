...
export async function GET(_req: NextRequest, { params }: RouteParams) {
  try {
    ...
    const { data: profile, error } = await supabase
      .from("profiles")
      .select(
        "id, username, age, show_age, age_verified, gender_identity, bio, interests, languages, avatar_url, avatar_emoji, reputation_score, chat_streak, positive_ratings, created_at"
      )
      .eq("id", friendId)
      .maybeSingle();
