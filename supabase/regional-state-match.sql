-- Optional US state filter for regional matchmaking (whole country when state is null)
...
alter function find_or_create_match(uuid, text, text, boolean, text) security definer;
alter function find_or_create_match(uuid, text, text, boolean, text) set search_path = public;
