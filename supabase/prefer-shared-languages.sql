-- Optional "prefer shared languages" matchmaking toggle

alter table waiting_users
  add column if not exists prefer_shared_languages boolean not null default false;
...
begin
  ...
  select gender_identity, looking_for, interests, languages
  into v_seeker_gender, v_seeker_looking_for, v_seeker_interests, v_seeker_languages
  from profiles
  where id = p_user_id;
...
alter function find_or_create_match(uuid, text, text, boolean, text, boolean) security definer;
alter function find_or_create_match(uuid, text, text, boolean, text, boolean) set search_path = public;
